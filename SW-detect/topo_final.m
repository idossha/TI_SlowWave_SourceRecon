...
output_dir = '/users/idohaber/desktop/Jan_23_Topoplots/biggest_wave/';
...
for i = 1:length(files)
    ...
    % first_wave = filtered_epochs_500ms_first
    % most_negative = filtered_epochs_500ms_most_negative
    % all_waves = sorted_slow_waves

    % Construct the path to the corresponding CSV file
    csv_path = sprintf('/volumes/csc-ido/analyze/%s/%s/output/strength_%s_%s_forsw/filtered_epochs_500ms_most_negative.csv', ...
    ...
    % ---- Modification Start ----
    % **Do not exclude rows with 'Region_Classification' == 'unclassified'**
